...
# This script is run for connection experiments by running "calculate_diffs_all.sh".

# Arguments:
# $1 - The folder containing all log files for a specific experiment
...
if [ $(find ${DIRECTORY} -maxdepth 1 -iname "error.log" | wc -w) -eq 1 ]; then
    echo "An error log is present in \"${DIRECTORY}\"! Experiment data are invalid! Skipping..."
    echo ""
    exit 1
fi
...
calculate_diffs () {
	...
	do
	    ...
	    metric_type=$(echo $first_file | awk -F "_" '{print $15}')

	    # The specific source of sampled data (e.g. application, intr, vmstatz)
	    metric_specific=$(echo $first_file | awk -F "_" '{print $16}' | awk -F "." '{print $1}')
...
aggregate_results () {
	...
	do
	    ...
	    newfile=$(echo "$filename" | awk -F "_" '{$11 = ""; $12 = ""; print}' | sed -r 's/ +/_/g')
	    cd ${DIRECTORY}
...
echo "Calculating diffs and aggregating for \"${DIRECTORY}\""
echo "Calculating connection establishment delay diffs (if available)..."
calculate_diffs_delay stats_time_establishment connection

echo "Calculating forloop diffs (if available)..."
calculate_diffs beforeforloop afterforloop forloop

echo "Calculating connection establishment diffs (before loop initialization)..."
calculate_diffs start afterallconnected startconnection

echo "Aggregate results..."

echo "Aggregate connection establishment results (before loop initialization)..."
aggregate_results startconnection

echo "Aggregate forloop results (if available)..."
aggregate_results forloop

echo "Producing tables.."
echo "Producing total CPU usage table..."
produce_table startconnection cpu_all_application

echo "Producing total memory usage table..."
produce_table startconnection memory_all_application

echo "Producing connection establishment delay table..."
produce_table connection time_establishment

echo "Producing delay table..."
produce_table startconnection time_application
