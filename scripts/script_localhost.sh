...
directories=("/home/helena/results-neat-test-suite/client/tcp" "/home/helena/results-neat-test-suite/server/tcp")
number_of_tests=10
...
test_mode="cpu"	# "connection" "memory" "cpu" 
...
for (( k = 0; k < "${#directories[@]}"; k+=2 )); do 	# tcp and sctp
	...
	for (( i = 0; i < "${#flows[@]}"; i++ )); do
		...
		for (( j = 0; j < "${number_of_tests}"; j++ )); do
			...
			./neat_server -C "$((transport[k+1] * flows[i]))" -a "${a}" -b "${b}" \
				-M "${transport[k]}" -I "${host}" -p "${port}" -v "${log_level}" -u "${log_level}" \
				>/dev/null 2>&1 & #&>>/home/helena/output_server.txt 2>&1 & #>/dev/null 2>&1 & #

			sleep 1		# to make sure the server is up when the client starts

			./neat_client -R "${directories[k]}" -A -s -C "${flows[i]}" -a "${a}" -b "${b}" \
				-M "${transport[k]}" -i "${host}" -n "${flows[i]}" -v "${log_level}" -u "${log_level}" "${host}" \
				"${port}" "${counter}" &>/dev/null & #&>>/home/helena/output_client.txt & #&>/dev/null & #
...
if [[ "${test_mode}" == "cpu" ]]; then
	...
	data_together_in_new_file "${directories[0]}" "jsondumps" "cpu"
	data_together_in_new_file "${directories[0]}" "jsonpack" "cpu"
	data_together_in_new_file "${directories[0]}" "jsonloads" "cpu"
	data_together_in_new_file "${directories[0]}" "jsondecref" "cpu"
	add_jsonfunctionsdata_together "${directories[0]}" "cpu"
elif [[ "${test_mode}" == "memory" ]]; then
	echo "Calculating the difference in memory usage for JSON functions..."
...
#produce_graph ${directories[0]} "cpu" "all" 0.000001
...
if [[ "${test_mode}" == "cpu" ]]; then
	...
	#produce_graph_compare ${directories[0]} "cpu" "connection" 0.000001 "json" "json"
elif [[ "${test_mode}" == "memory" ]]; then
	...
	#produce_graph_compare ${directories[0]} "memory" "connection" 1 "json" "json"
elif [[ "${test_mode}" == "connection" ]]; then
	produce_graph ${directories[0]} "cpu" "connection" 0.000001
	produce_graph ${directories[0]} "memory" "connection" 1
fi
