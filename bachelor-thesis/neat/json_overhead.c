...
char *cache_jsondumps(json_t *json, size_t flag, struct jsondumps_struct *a) 
{
	...
	if (!a->first_flow) {
		if (!a->full_array) {
			...
			what_index(a);
			json_decref(a->array[a->index].key);
		}
	} else {
		init_array(a);
		a->first_flow = false;
...
json_t *cache_jsonpack_two_values(char *compare_value, char *compare_value2, struct jsonpack_two_keys_struct *a) 
{
	...
	if (!a->first_flow) {
		if (!a->full_array) {
			...
			what_index(a);
			json_decref(a->array[a->index].endpoint_before);
		}
	} else {
		init_array(a);
		a->first_flow = false;
...
json_t *cache_jsonpack_uint_key(uint16_t compare_value, struct send_prop_to_pm_struct *a) 
{
	...
	if (!a->first_flow) {
		if (!a->full_array) {
			...
			what_index(a);
			json_decref(a->array[a->index].port_before);
		};
	} else {
		init_array(a);
	}
...
json_t *cache_jsonpack_char_key(const char *compare_value, struct jsonpack_char_key_struct *a) 
{
	...
	if (!a->first_flow) {
		if (!a->full_array) {
			...
			what_index(a);
			json_decref(a->array[a->index].address_before);
		}
	} else {
		init_array(a);
		a->first_flow = false;
...
char *cache_jsondumps2(json_t *json, size_t flag, struct send_prop_to_pm_struct *a) 
{
	...
	if (!a->first_flow) {
		for (unsigned i = 0; i < a->used; ++i) {
			if (json_equal(a->array[i].json_value, json)) {
				return a->array[i].output_buffer_before;
			}
		}
	} else {
...
json_t *cache_jsonpack_four_values(char *compare_ip, unsigned int compare_port, 
	int compare_transport, _Bool compare_result, struct send_result_conn_struct *a) 
{
	...
	if (!a->first_flow) {
		for (unsigned i = 0; i < a->used; ++i) {	
			if (strcmp(compare_value, a->array[i].interface_before) == 0) {	 
				result_obj_temp = a->array[i].result_obj_before;	
				a->same_value = true;
