...
sample(char *name, int condition)
{
	...
	if (condition) {
	    sample_usage_data(config_prefix, name, "/home/helena/results-neat-test-suit/client/tcp");
        /*if (sample_usage_data(config_prefix, name, "/home/helena/results-neat-test-suit/client/tcp") == -1) {
		set_error_time(1);
	    }*/
	}
    //}
...
void
subtract_time(struct timespec *start_t, struct timespec *end_t, struct timespec *diff)
{
    if ((end_t->tv_nsec - start_t->tv_nsec) < 0) {
        diff->tv_sec = end_t->tv_sec - start_t->tv_sec - 1;
        diff->tv_nsec = end_t->tv_nsec - start_t->tv_nsec + 1000000000;
    } else {
        diff->tv_sec = end_t->tv_sec - start_t->tv_sec;
        diff->tv_nsec = end_t->tv_nsec - start_t->tv_nsec;
    }
}
