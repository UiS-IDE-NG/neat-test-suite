...
TPCONF_test_id = 'noprefix' + '_neatnomultconnectionhesctptcpminimal'
...
traffic_custom = [
    ('0.0', '1', " start_custom_traffic,"
     ...
     " duration=V_duration,"
     " add_prefix='1',"
     " parameters='-R %s -A -s -a %s -b %s -C %s -I %s -M %s -p %s -v %s -u %s' % ('fredhau/testhost2-freebsd/neat/no-mult/connection/he/listen-sctptcp/minimal_he_delay', V_flows, V_flows, str(int(V_server_connections) * int(V_flows)), '172.16.10.2', V_server_transports, 12327, 1, 2)"),
    ('2.0', '2', " start_custom_traffic,"
     ...
     " duration=V_duration,"
     " add_prefix='1',"
     " parameters='-R %s -A -s -d %s -C %s -a %s -b %s -i %s -l %s -M %s -n %s -v %s -u %s %s %s' % ('fredhau/testhost5-freebsd/neat/no-mult/connection/he/listen-sctptcp/minimal_he_delay', '260', V_flows, '0', V_flows, '172.16.11.4', 10240, V_client_transports, V_flows, 1, 2, '172.16.10.2', 12327)"),
]
...
TPCONF_server_transports = [('SCTP-TCP', '2')]
TPCONF_client_transports = [('SCTP-TCP', '2')]
...
#   Vary name		V_ variable	  file name	values			extra vars
    ...
    'servertransports' : (['V_server_transports', 'V_server_connections'], ['servertransports', 'serverconnections'], TPCONF_server_transports, {}),
    'clienttransports' : (['V_client_transports', 'V_client_connections'], ['clienttransports', 'clientconnections'], TPCONF_client_transports, {}),
}
...
#   V_ variable			value
    ...
    'V_server_transports' :     TPCONF_server_transports[0][0],
    'V_client_transports' :     TPCONF_client_transports[0][0],
    'V_server_connections' :    TPCONF_server_transports[0][1],
    'V_client_connections' :    TPCONF_client_transports[0][1],
}

TPCONF_vary_parameters = ['flows', 'data_sizes', 'clienttransports', 'runs']
