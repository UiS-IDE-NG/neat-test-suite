...
TPCONF_test_id = 'noprefix' + '_neatnomultconnectionhesctp'
...
traffic_custom = [
    ('0.0', '1', " start_custom_traffic,"
     ...
     " duration=V_duration,"
     " add_prefix='1',"
     " parameters='-R %s -A -s -a %s -b %s -C %s -I %s -M %s -p %s -v %s -u %s' % ('fredhau/testhost2-freebsd/neat/no-mult/connection/he/listen-sctp', '0', V_flows, str(int(V_server_connections) * int(V_flows)), '172.16.10.2', V_server_transports, 12327, 1, 2)"),
    ('2.0', '2', " start_custom_traffic,"
     ...
     " duration=V_duration,"
     " add_prefix='1',"
     " parameters='-R %s -A -s -C %s -a %s -b %s -i %s -l %s -M %s -n %s -v %s -u %s %s %s' % ('fredhau/testhost5-freebsd/neat/no-mult/connection/he/listen-sctp', V_flows, '0', V_flows, '172.16.11.4', 10240, V_client_transports, V_flows, 1, 2, '172.16.10.2', 12327)"),
]
...
TPCONF_server_transports = [('SCTP', '1')]
