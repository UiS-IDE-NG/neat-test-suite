...
TPCONF_test_id = 'noprefix' + '_neatnomulttransferuploadtcp'
...
traffic_custom = [
    ('0.0', '1', " start_custom_traffic,"
     ...
     " duration=V_duration,"
     " add_prefix='1',"
     " parameters='-R %s -A -H %s -a %s -b %s -s -C %s -D %s -I %s -M %s -p %s -v %s -u %s' % ('fredhau/testhost2-freebsd-transfer/neat/no-mult/transfer/upload/tcp', '1000', V_flows, '0', str(int(V_connections) * int(V_flows)), V_flows, '172.16.10.2', V_transports, 12327, 1, 2)"),
    ('10.0', '2', " start_custom_traffic,"
     ...
     " duration=V_duration,"
     " add_prefix='1',"
     " parameters='-R %s -A -h %s -a %s -b %s -s -C %s -D %s -i %s -l %s -M %s -n %s -v %s -u %s %s %s' % ('fredhau/testhost5-freebsd-transfer/neat/no-mult/transfer/upload/tcp', V_data_size, V_flows, '0', V_flows, V_flows, '172.16.11.4', 10240, V_transports, V_flows, 1, 2, '172.16.10.2', 12327)"),
]
